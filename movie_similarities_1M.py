...
def loadMovieNames():
    movieNames = {}
    with open("./datasets/ml-1m/movies.dat", encoding="ascii", errors="ignore") as f:
        for line in f:
            fields = line.split("::")
...
data = sc.textFile("./datasets/ml-1m/ratings.dat")
...
if len(sys.argv) > 1:
    ...
    for result in results:
        ...
        print(
            f"{nameDict[similarMovieID]}\tScore: {str(sim[0])}\tstrength: {str(sim[1])}"
        )
