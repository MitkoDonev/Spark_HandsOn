...
movies = (
    spark.read.option("sep", "\t").schema(moviesSchema).csv("./datasets/ml-100k/u.data")
)
...
if len(sys.argv) >= 1:
    ...
    for result in results:
        ...
        print(
            f"{getMovieName(movieNames, similarMovieID)}\tScore: {str(result.score)}\tStrength: {str(result.numPairs)}"
        )
